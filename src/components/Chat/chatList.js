...
import { handleChatList, handleOnlineUser } from '../../redux/actions/chatActions';
...
const ChatList = () => {
    ...
    try {
      ...
      if (onlineUsers.length > 0) {
        const updatedChatList = chatList.map(item1 => {
          const matchingItems = onlineUsers.find(item2 => item2.userId === item1.key);
        ...
        console.log('ipdated', updatedChatList);
  ...
  const handleChatRedirect = async item => {
    console.log(item);
    await handleCreateChat(item.key, navigate, dispatch);
  };
